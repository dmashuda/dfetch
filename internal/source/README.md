...
subpackage (`internal/source/github`, `internal/source/jaeger`).
...
`planScan` (`internal/engine/plan.go`) decides what to offer each source in the
...
`builtins` map in `internal/engine/engine.go` (and import the package):
...
**`internal/source/jaeger`** — Jaeger api_v3 returning the OTLP model. Shows:
...
1. **Create the package** `internal/source/<name>/<name>.go`: a `Connector`
